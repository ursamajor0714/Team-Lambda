// =============================================================================
// [Node 전환] middleware/csrf.js — CSRF 공격을 막는 "손목밴드(토큰)" 검사기
// -----------------------------------------------------------------------------
// 📌 CSRF가 뭐냐 (복습):
...
//
// 📌 Django와 비교:
//   - Django는 CsrfViewMiddleware가 자동으로 처리해줬다.
//   - Node(Express)엔 기본 내장이 아니라서 여기서 직접 만든다.
...
export function issueCsrfToken(req, res) {
  ...
  //   (Django의 CSRF_COOKIE_HTTPONLY=False 와 동일한 이유)
...
export function csrfProtection(req, res, next) {
  ...
  if (!cookieToken || cookieToken !== headerToken) {
    // 토큰이 없거나 서로 다르면 거절 — Django는 이때 403 Forbidden을 돌려줬다.
